...
//    with must_change_password=1 so first login forces a fresh password.
{
  ...
  if (userCount === 0) {
    ...
    if (cfg && cfg.admin_password_hash) {
      ...
      console.log('[OpenFalcon] Migrated legacy admin password to user "admin"');
    } else {
      ...
      db.prepare(`
        ...
        VALUES ('admin', ?, 1, 1)
      `).run(defaultHash);
      console.log('[OpenFalcon] Created default user "admin" with password "admin" — change it on first login!');
    }
  }
}
