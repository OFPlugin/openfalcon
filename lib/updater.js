...
// Skips drafts and pre-releases. Returns null if list is empty or
// contains no semver-shaped tags.
function pickHighestSemverRelease(releases) {
  if (!Array.isArray(releases) || releases.length === 0) return null;
  const candidates = releases.filter(r => r && r.tag_name && !r.draft && !r.prerelease);
  if (candidates.length === 0) return null;
  candidates.sort((a, b) => compareVersions(b.tag_name, a.tag_name)); // descending
  return candidates[0];
}

// Try /releases?per_page=100 first (richer data — release notes, published
// date, name). We pick the highest semver ourselves so GitHub's manually-
// set "latest" marker doesn't matter. Falls back to /tags if there are no
// Release objects at all.
async function fetchLatestRelease() {
  const releases = await ghGetJson(GITHUB_RELEASES_URL);
  if (releases.status === 200 && Array.isArray(releases.body) && releases.body.length > 0) {
    const best = pickHighestSemverRelease(releases.body);
    if (best) {
      return {
        tag: best.tag_name,
        name: best.name || best.tag_name,
        url: best.html_url || GITHUB_RELEASES_WEB_URL,
  ...
  // 404 or empty releases list means no Release objects exist — fall back
  // to tags. Other non-200s (rate limit, server error) we treat the same.
  ...
  return {
    tag: highest,
    name: highest,
    // GitHub's canonical per-tag landing page is /releases/tag/<name>.
    // It works even when no Release object exists (GitHub renders a
    // tag-info view: commit, files, source downloads). The /tags/<name>
    // form is NOT a real route — /tags is a list-only page. Verified
    // against ShowPilotFPP/ShowPilot before shipping this fix.
    url: `${GITHUB_RELEASES_WEB_URL}/tag/${encodeURIComponent(highest)}`,
    publishedAt: null,
    body: '',
    source: 'tags',
  };
