...
function renderPlaylistGrid(sequences, mode, voteCounts) {
  ...
  const rows = sequences.map(seq => {
    ...
    // width/height HTML attributes are presentational hints with the LOWEST
    // CSS cascade precedence — any author CSS that targets `.sequence-image`,
    // `.jukebox-list img`, `.cell-vote-playlist img`, etc. will override them.
    // We add them so RF templates that DON'T declare image rules (4 of RF's
    // 6 default templates: dynamic-menu styles via descendants but most don't
    // style at all) still render images at a sensible thumbnail size instead
    // of the album-art's native ~300×300, which dominates the row and breaks
    // the page's vertical rhythm. 40 matches `.sequence-image { height: 40px }`
    // in the-og and lumos-light-show, the canonical RF default.
    const artImg = bustedUrl
      ? `<img class="sequence-image" data-seq-name="${safeNameAttr}" src="${escapeHtml(bustedUrl)}" alt="" width="40" height="40" loading="lazy" />`
      : '';
...
function renderTemplate(template, state) {
  ...
  //
  // We determine the enclosing container by scanning backwards from
  // each {PLAYLISTS} occurrence for the most recent
  // `data-showpilot-container="X"` marker that hasn't been closed
  // by a balancing `</div>` after it. If none is found (or the
  // container isn't jukebox/voting), we fall back to the active
