...
`;
    // SW registration script. Wrapped so a failed register doesn't break
    // anything else on the page. Captures the install prompt so we can
    // expose a manual "Install" button later if someone wants to add one.
    const pwaScript = `
  <script>
  ...
  // Capture beforeinstallprompt so the user (or a custom button) can
  // trigger install at the right moment instead of relying on browser
  // auto-prompts (which Chrome heavily throttles).
